  ...
  const getExperiences = useCallback(async () => {
    try {
      const res: AxiosResponse<any> = await axios.get(
        `${BASE_API_URL}/experiences/user/${user._id}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );
      if (res.status === 200) {
        localStorage.setItem(
          "experiences",
          JSON.stringify(res.data.userExperience.experiences)
        );
  ...
  return (
    <div className="bg-white shadow rounded px-8 pt-6 pb-8 ml-0 lg:ml-0 my-5">
      <div className="flex items-center justify-between flex-wrap">
        <h2 className="text-slate-700 text-md md:text-xl xl:text-2xl font-bold mb-0">
          Experience
        </h2>
      ...
      {experiences && experiences.length ? (
        experiences.map((experience: any) => (
          <div
            key={experience._id}
            className="flex flex-wrap items-center border-b last:border-b-0 pb-6 last:pb-0 mt-7"
          >
            <img
              src={experience.companyLogo}
              className="w-[50px] h-[50px] object-cover block ml-auto mr-auto sm:mr-0 sm:ml-0 border-2 border-slate-300 rounded-full p-1"
                ...
                <p className="text-sm text-slate-400">
                  ...
                  {experience.endDate
                    ? new Date(experience.endDate).getDate() +
                      " " +
                      months[new Date(experience.endDate).getMonth()] +
                      ", " +
