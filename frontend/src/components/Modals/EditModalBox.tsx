  ...
  const userInfoUpdate = async () => {
    ...
    try {
      const res: AxiosResponse<any> = await axios.patch(
        `${BASE_API_URL}/auth/user/${user._id}`,
        { age, userExperience, phone, userLocation },
  ...
  return (
    <>
      <button
        ...
        onClick={onOpenModal}
        role="edit"
        className="text-sm md:text-md text-slate-500 border-2 border-slate-500 px-3 py-2 ml-6 lg:ml-8 rounded hover:bg-slate-600 hover:text-slate-100 transition ease duration-500"
      >
        <FaRegEdit />
      </button>
