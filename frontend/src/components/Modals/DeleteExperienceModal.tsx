    ...
    try {
      ...
      if (res.status === 200) {
        ...
        user.experiences = user.experiences.filter(
          (item: any) => item._id !== res.data.experience._id
        );
  ...
  return (
    <>
      <button
        type="button"
        onClick={onOpenModal}
        className="text-red-500 basis-[3%] border-2 border-red-500 px-3 py-2 ml-6 rounded hover:bg-red-600 hover:text-red-100 transition ease duration-500"
      >
        <FaTrashAlt />
      </button>
      <Modal open={open} onClose={onCloseModal} center>
        ...
        <div className="text-center mt-5 mb-2">
          <button
            ...
            className="text-sm bg-teal-500 text-white font-bold px-5 py-2 rounded"
          >
            Yes
          </button>
          <button
            type="reset"
            onClick={handleReset}
            className="text-sm bg-orange-500 text-white font-bold px-5 py-2 rounded ml-6"
          >
            No
          </button>
        </div>
